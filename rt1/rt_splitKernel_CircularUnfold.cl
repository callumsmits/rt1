...
//    rayStart = viewOrigin;
    ...
    xangle = column / (float)width * 2 * M_PI_F - M_PI_F;
    yangle = row / (float)height * M_PI_F;
    rayStart.x = sin(xangle) * sin(yangle) * circScale + lookAt.x;
    rayStart.y = cos(yangle) * circScale + lookAt.y;
    rayStart.z = -cos(xangle) * sin(yangle) * circScale + lookAt.z;
    rayStart.w = 1;
    
//    if ((i > 500000) && (i < 501000)) {
//    if (column == 20)
//        printf("i: %d, xangle: %.2f yangle: %.2f raystart: %.2f %.2f %.2f %.2f\n", i, xangle, yangle, rayStart.x, rayStart.y, rayStart.z, rayStart.w);
//    }
